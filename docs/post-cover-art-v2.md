...
> **정정(2026-09-14):** 아래는 당시 제작 이력이다. NULL 사례의 ‘변경 판정 단일화’와 ‘항목 소유권’ 설명은 실제 수행으로 확인되지 않아 정정했다. 이를 표현한 ONE CHECK 표지는 더 이상 글에 연결하지 않으며, 빈 값 비교와 처리 구분값 규약이라는 두 오류를 표현하는 코드형 표지로 교체했다. 기존 이미지 파일은 이력 보존용이며 재사용하지 않는다. 주소·배치 표지는 유지한다.
