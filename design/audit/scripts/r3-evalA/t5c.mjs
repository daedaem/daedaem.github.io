...
for (const [vp,w,h] of [['d',1280,900],['m',390,844]]) for (const s of ['new','cur']) {
  ...
  {const bb=await p.evaluate(()=>{const b=[...document.querySelectorAll("header button")].find(b=>/모드/.test(b.getAttribute("aria-label")||"")&&b.getBoundingClientRect().height>0);const r=b.getBoundingClientRect();return [r.x+r.width/2,r.y+r.height/2]}); await p.mouse.click(bb[0],bb[1]);}
