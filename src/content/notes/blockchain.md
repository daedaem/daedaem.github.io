...
> **바로잡음(2026-09-04):** 사토시가 할 핀니에게 10 BTC를 보낸 첫 개인 간 비트코인 거래는 **2009년 1월 12일**이다.
# 2. 블록체인 배경

## 2-1 .**기존 중앙집중형 시스템의 문제**

> **이미지 안내(2026-09-04):** 외부 기사 이미지가 더 이상 안정적으로 표시되지 않아 임베드를 제거했다. [출처 기사 보기](https://www.fntimes.com/html/view.php?ud=20160425003812151975_18&mobile=1)
...
> **이미지 안내(2026-09-04):** 외부 이미지 임베드를 제거했다. [ETRI 원문 보기](https://www.etri.re.kr/webzine/20190329/sub01.html)
...
> **이미지 안내(2026-09-04):** 외부 이미지 임베드를 제거했다. 각 블록이 자신의 데이터와 이전 블록의 해시를 포함해 연결되는 구조를 나타낸 그림이었다.
...
> **이미지 안내(2026-09-04):** 외부 이미지 임베드를 제거했다. 블록 내용이 바뀌면 현재 해시가 바뀌고 다음 블록의 이전 해시와 맞지 않게 되는 과정을 나타낸 그림이었다.

## 3-2. 🔨채굴

[노마드코더 - "개발자가 설명해주는 비트코인 채굴 방법 (ft 시뮬레이터)"](https://www.youtube.com/watch?v=ElGBP90XZWE)

~~블록체인에 들어오는 거래 내역을 검증하고 질문의 답을 찾아 블록을 체인에 올리면서 이를 통해 받는 일종의 수수료가 코인이다.~~

> **바로잡음(2026-09-04):** 비트코인 채굴자는 유효한 블록을 만들면 새로 발행되는 **블록 보조금**과 그 블록에 포함한 거래의 **수수료**를 보상으로 받는다. 코인 자체를 단순히 수수료라고 부르면 둘을 혼동한다.
...
> **바로잡음(2026-09-04):** 작업 증명은 블록 헤더의 해시가 네트워크 목표값보다 작아지도록 반복 계산하고, 그 계산 비용을 네트워크가 쉽게 검증하는 방식이다. 채굴자는 `nonce`뿐 아니라 코인베이스의 extra nonce 등 헤더에 영향을 주는 값도 바꾼다.
...
~~전 세계 사람들이 사용하는 네트워크에 내 코드를 공유하고 검증·실행할 수 있지만 변경은 불가능하다.~~

> **바로잡음(2026-09-04):** 배포된 컨트랙트 코드는 보통 직접 덮어쓰지 못하지만, 프록시 패턴이나 새 컨트랙트로의 이전을 통해 업그레이드 가능한 구조를 설계할 수 있다. 그리고 모든 블록체인이 공개 네트워크인 것도 아니다.
...
~~블록체인 기술을 사용하면 중개자가 더 이상 필요하지 않은데, 이는 분산화된 네트워크 노드가 마이닝을 통해 거래를 검증하기 때문이다.~~ 이러한 이유로 블록체인은 종종 신뢰가 필요 없는 시스템(trustless system)으로 지칭됩니다.

> **바로잡음(2026-09-04):** 중개자가 항상 사라지는 것은 아니며, 모든 체인이 마이닝을 쓰지도 않는다. 신뢰 대상이 단일 기관에서 합의 규칙·검증자·클라이언트 소프트웨어·오라클 등으로 바뀐다고 보는 편이 정확하다.
...
> **이미지 안내(2026-09-04):** 외부 기사 이미지가 더 이상 안정적으로 표시되지 않아 임베드를 제거했다.
...
~~이러한 대출 서비스는 공용 블록체인 위에 구축되기 때문에 암호화된 검증 방법으로 보장되며, 거래 당사자의 위험을 줄이고 더 저렴하고 빠르게 많은 사람이 이용할 수 있게 한다.~~

> **바로잡음(2026-09-04):** 공개 블록체인은 거래 기록과 규칙 실행을 검증할 수 있게 하지만, 스마트 컨트랙트 버그·오라클·담보 청산·관리 키·수수료 위험까지 없애지는 않는다. 비용과 속도도 네트워크 혼잡과 설계에 따라 달라진다.
...
~~NFT란 JPG, GIF, 비디오 등의 디지털 파일에 대한 소유권을 블록체인상에 저장함으로써 위·변조가 불가능한 상태로 영구 보존하고, 그 소유권을 탈중앙화된 형태로 확인할 수 있도록 해놓은 것이다.~~

> **바로잡음(2026-09-04):** NFT는 블록체인에 기록된 고유 토큰이다. 토큰을 보유한다고 원본 파일의 저작권이나 법적 소유권이 자동으로 이전되지는 않는다. 이미지와 메타데이터가 외부 URL에 저장된 경우 파일의 영구 보존도 보장되지 않는다.
...
> **바로잡음(2026-09-04):** 비트코인과 이더리움의 ETH는 대체 가능하지만 ERC-20 토큰은 아니다. ERC-20과 ERC-721은 **이더리움 위 토큰 표준**이고, ETH는 이더리움 프로토콜의 네이티브 자산이다. NFT도 반드시 ERC-721인 것은 아니며 ERC-1155 등 다른 표준이 있다.
...
class BlockChain:
    ...
    def isValid(self):
        ...
        while i<len(self.chain):
            # 현재 블록의 해시값과 계산된 해시값 비교
            if self.chain[i].hash != self.chain[i].calHash():
                return False
            # 현재 블록의 이전 해시값과 직전 블록에 저장된 해시값 비교
...
> **보충(2026-09-04):** 이 코드는 해시 연결과 작업 증명의 모양을 보는 학습 예제다. 문자열을 구분자 없이 이어 붙여 해시하므로 직렬화가 모호할 수 있고, 분산 합의·서명·네트워크 검증·난이도 조정이 없어 실제 블록체인 구현으로 사용할 수 없다. `vars(block)`은 객체의 `__dict__`를 돌려주며 메서드까지 반환하는 함수가 아니다. `time.time()`의 소수부는 초 미만을 표현하지만 마이크로초 정확도를 보장한다는 뜻은 아니다.
