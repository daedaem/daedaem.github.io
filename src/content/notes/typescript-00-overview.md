...
- 장점
  - **타입**을 이용하면서 작동방식에 대해 명시적이어야 하며 예상치 못한 에러를 피할 수 있음
  - 타입스크립트는 컴파일 과정에서 에러를 잡아내기 때문에, 기존에 런타임에서 발생하던 에러를 개발단계에서 핸들링할 수 있게 된다.
  - 구형 브라우저에서 사용 가능한 자바스크립트 코드로 컴파일되므로 next-generation 자바스크립트 기능도 사용할 수 있다.
...
> **바로잡음(2026-09-05):** 브라우저와 Node.js를 구분해야 한다. 아래 보충처럼 지원 버전의 Node.js는 일부 TypeScript 문법을 타입 제거 후 실행할 수 있다. 이 기능은 타입 검사를 대신하지 않는다.

- 타입스크립트는 브라우저에서 실행할 수 없기 때문에 항상 js 파일을 가져와야 한다.

> **보충(2026-09-04):** Node.js는 22.6에서 타입 제거 기능을 추가했고, 22.18부터 기본으로 켰다. 다만 타입 검사와 `tsconfig.json` 변환을 하지 않으며, enum처럼 자바스크립트 생성이 필요한 문법과 `.tsx`에는 제약이 있다. 브라우저는 여전히 TypeScript 소스를 직접 실행하지 않는다.
...
- 아래 2, 3이 문자열이기 때문에 원하는 5가 아닌 ‘23’이 된다. 이는 기능적 오류가 아니라서 오류가 발생하지 않는다. 논리적 오류다.
...
- 컴파일 시에 에러 메시지로 오류의 위치를 알려주기 때문에 찾기 쉬운 에러
- ~~수정되지 않으면 프로그램은 컴파일되지 않음~~

> **바로잡음(2026-09-05):** TypeScript는 타입 오류가 있어도 기본 설정에서는 JavaScript 파일을 내보낼 수 있다. 오류 시 출력을 막으려면 `noEmitOnError: true`를 설정한다. [공식 문서](https://www.typescriptlang.org/tsconfig/noEmitOnError.html)
- ex) 선언되지 않은 변수 사용, 타입 불일치 등

### 런타임 에러(Runtime Error)

- 컴파일 후 프로그램이 실행되는 동안에 발생
- 프로그램 설계 미숙, 기계적 결함 등으로 발생
