...
> **바로잡음(2026-08-30):** 저 인용은 **시맨틱 웹**(RDF·온톨로지로 기계가 읽는 데이터의 웹, W3C의 별도 프로젝트)에 대한 문장이고, 이 글의 주제인 **시맨틱 HTML**(의미 있는 태그 사용)과는 다른 개념이다. w3schools가 둘을 섞어 써서 그대로 옮긴 것이다. 시맨틱 HTML의 근거는 아래 SEO·접근성·가독성 세 가지로 충분하다.

### 1. 검색엔진 최적화(search engine optimization, **_SEO_**)

- **검색 엔진이 태그의 목적에 부합하게 설계되어 있는 구조의 사이트에서 더욱 빨리 효율적으로 정보를 파악할 수 있어 검색 결과의 노출에 유리할 수 있게 해준다.** 검색엔진이 알맞은 검색결과를 내기 위해 웹사이트를 크롤링할 때, 웹사이트의 내부에 담긴 정보를 기반으로 페이지 내 검색 키워드의 우선순위를 판단하게 된다.
- ~~유튜브 영상에서 본 바로는 한 페이지에 h1 태그가 하나씩은 반드시 들어가 있는 게 좋다고 한다.~~ `<h1>` 태그는 헤드라인을 의미하고, 일반적으로 페이지의 주제를 파악하는 데 도움이 된다.

> **바로잡음(2026-09-04):** HTML 표준은 페이지당 `<h1>`을 정확히 하나로 제한하지 않는다. 문서의 주 제목을 명확히 하고 제목 단계(`<h1>`~`<h6>`)를 논리적으로 구성하는 것이 핵심이다.
- Ex) image 를 **div**에 background-image 로 붙이는것과 **img** 태그를 쓰는것중 **img** 태그를 쓰는것이 검색 엔진이 의미적으로 이미지라는 것을 알 수 있어서 효율적이다.
...
> **이미지 안내(2026-09-04):** 외부 블로그 이미지가 만료되거나 로딩을 막을 수 있어 임베드를 제거했다. 태그의 역할은 본문 설명과 위의 로컬 도해에서 확인할 수 있다.
