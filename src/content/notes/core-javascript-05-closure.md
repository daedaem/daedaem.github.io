...
legacyPath: "/230216_코어자바스크립트 ch 5. 클로저/"
---
...
> **바로잡음(2026-09-04):** 엔진이 “나중에 쓸 변수”라고 판단하거나 단순 참조 카운트만 세는 것은 아니다. 반환된 함수에서 `_name`과 `_logged`에 계속 도달할 수 있으므로 그 어휘 환경이 GC 대상이 되지 않는 것이다.
...
- ~~보통 ‘메모리 누수’라는 표현은 개발자 의도와 달리 어떤 값의 참조 카운트가 0이 되지 않아 GC의 수거 대상이 되지 않는 경우다. 따라서 의도적으로 참조 카운트를 0이 되지 않게 설계한 경우는 누수가 아니다.~~

> **바로잡음(2026-09-04):** 현대 자바스크립트 GC는 단순 참조 카운트가 아니라 루트에서 **도달 가능한지**를 기준으로 객체를 추적한다. 메모리 누수는 더 이상 필요하지 않은 값이 타이머·이벤트 리스너·전역 컬렉션 같은 참조 때문에 계속 도달 가능한 상태로 남는 경우다.
- ~~최근 JS엔진에서는 과거 엔진에서 의도치 않게 누수가 발생하는 여러 상황(순환 참조, 이벤트 핸들러)이 발생하지 않거나 발견하기 힘듦.~~

> **바로잡음(2026-09-05):** 도달할 수 없는 순환 참조는 수거할 수 있지만, 수명이 긴 이벤트 대상·타이머·캐시가 불필요한 값을 계속 참조하는 누수는 현대 엔진에서도 발생한다. GC가 그 값의 업무상 필요 여부까지 판단하지는 않는다.
  - cf. 순환 참조
...
- 따라서 오래 유지되는 타이머와 이벤트 리스너가 필요 없어지는 시점에 해제되는지 확인한다.

### **클로저 + 메모리 관리법**

- ~~참조 카운트를 0으로 만드는 방법은 식별자에 기본형 데이터(보통 null 또는 undefined)를 할당하는 것이다.~~

> **바로잡음(2026-09-04):** 변수에 `null`을 넣는 것은 참조 하나를 끊는 방법일 뿐, 다른 참조가 남아 있으면 수거되지 않는다. `clearInterval`, `removeEventListener`처럼 값을 붙잡고 있는 소유자에서 등록을 해제하는 것이 먼저다.
- 예제5. 클로저의 메모리 관리
...
- ~~클로저는 그 본질이 메모리를 계속 차지하는 개념이므로 더는 사용하지 않게 된 클로저는 직접 메모리를 관리해야 한다.~~

> **바로잡음(2026-09-04):** 클로저 자체가 누수는 아니다. 클로저에 더 이상 도달할 수 없으면 캡처한 환경도 GC 대상이 된다. 수명이 긴 타이머·리스너·캐시가 불필요한 클로저를 계속 참조하지 않는지만 관리하면 된다.
