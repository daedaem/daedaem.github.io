...
legacyPath: "/230228_코어자바스크립트 ch 6. 프로토 타입/"
---
...
![prototype schematic](./coreJavaScript/prototype_schematic.png)
...
> **바로잡음(2026-09-05):** 일반적인 생성자 호출에서 인스턴스의 내부 `[[Prototype]]`이 생성자의 `prototype` 객체로 연결된다. `__proto__`라는 자체 프로퍼티가 자동 생성되는 것은 아니다. 확인에는 `Object.getPrototypeOf(instance)`를 사용한다.
...
> **바로잡음(2026-09-05):** 체인을 따라 프로퍼티를 찾는 과정이 프로토타입 체이닝이다. 일반 객체는 `Object.prototype`을 거치지만, `Object.create(null)`처럼 그렇지 않은 객체도 있다. 체인의 끝은 `null`이다.
...
> **바로잡음(2026-09-05):** `null`과 `undefined`에는 메서드를 호출할 수 없으며, null 프로토타입 객체는 `Object.prototype`의 메서드를 상속하지 않는다. 모든 값에서 직접 호출할 수 있다는 뜻은 아니다.
