...
- 각 **셀은 고유의 메모리 주소(memory address)를 갖고** 메모리 주소는 **메모리 공간의 위치**를 나타내며 메모리 크기만큼 정수로 표현된다. ex) 4GB 메모리는 0부터 4,294,967,295(0x00000000 ~ 0xFFFFFFFF)까지 메모리 주소를 갖는다.
...
- 프로그래밍 언어는 기억하고 싶은 값을 메모리에 저장하고 저장된 값을 재사용하기 위해 변수라는 메커니즘 제공
...
- 자바스크립트는 인터프리터에 의해 한 줄씩 순차적으로 실행되므로 console.log(score)가 참조 에러가 발생할 것처럼 보인다. 하지만, undefined가 출력된다.
- 이는 변수 선언이 소스코드가 한 줄씩 순차적으로 실행되는 시점, 즉 런타임(runtime)이 아니라 그 이전 단계에서 소스코드 평가 과정에서 모든 선언문을 먼저 실행하기 때문이다.
...
- ~~자바스크립트는 가비지 컬렉터를 내장한 매니지드 언어로서 가비지 컬렉터를 통해 메모리 누수를 방지한다.~~

> **바로잡음(2026-09-04):** GC는 도달할 수 없는 메모리를 자동 회수하지만, 더 이상 필요하지 않은 객체가 전역 변수·리스너·타이머·캐시에서 계속 참조되면 회수하지 못한다. 자동 GC가 있어도 메모리 누수는 생길 수 있다.

### cf. 언매니지드 언어(unmanaged language)와 매니지드 언어(managed language)
