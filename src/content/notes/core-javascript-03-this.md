...
- ~~this는 전역 객체를 가리킨다. window(브라우저에서) / global(node에서)~~

> **바로잡음(2026-09-04):** 브라우저의 일반 스크립트 최상위 `this`는 `window`지만 ES 모듈 최상위에서는 `undefined`다. Node.js CommonJS 모듈 최상위 `this`는 `global`이 아니라 `module.exports`이고, ES 모듈에서는 `undefined`다.
...
> **바로잡음(2026-08-30):** 전역 객체가 되는 것은 **비엄격 모드**에서만이다. `'use strict'`나 ES 모듈(항상 엄격 모드)에서는 일반 함수 호출의 this가 <strong><code>undefined</code></strong>다. 요즘 코드는 대부분 모듈이라 실제로는 undefined인 경우가 더 많다.
...
> **바로잡음(2026-09-04):** 콜백의 `this`는 콜백을 호출하는 API와 호출 방식에 따라 달라진다. 일반 함수 호출이면 비엄격 모드에서 전역 객체, 엄격 모드에서 `undefined`이고, 화살표 함수는 바깥 `this`를 사용한다.
