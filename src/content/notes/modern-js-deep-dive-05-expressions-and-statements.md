...
legacyPath: "/230319_모던 JS Deep Dive - 5.표현식과 문/"
---
...
- 사람이 이해할 수 있는 아라비아 숫자를 사용해 숫자 리터럴 3을 코드에 기술하면 자바스크립트 엔진은 이를 평가해 숫자 값 3을 생성.
  ...
  ![literal](<./modern js deep dive/5literal.jpeg>)

- 자바스크립트 엔진은 코드가 실행되는 시점인 런타임에 리터럴을 평가해 값을 생성
- 리터럴은 값을 생성하기 위해 미리 약속한 표기법.

  ![literal table](<./modern js deep dive/5literal_table.jpeg>)
...
- **토큰(token)** 이란 문법적인 의미를 가지며, **문법적으로 더 이상 나눌 수 없는 코드의 기본 요소**를 의미
  ...
  ![token](<./modern js deep dive/5token.jpeg>)
...
- 자바스크립트 엔진이 소스 코드 해석 시, 문의 끝이라고 예측되는 지점에 세미콜론 자동 삽입 기능(ASI, automatic semicolon insertion)이 암묵적으로 수행됨으로써 자동으로 세미콜론을 붙여준다.
...
- ~~세미콜론을 반드시 붙여야 하는지에 대한 의견이 나뉘지만, ESLint 같은 정적 분석 도구에서 세미콜론 사용을 기본으로 설정하고 있고 TC39도 세미콜론 사용을 권장한다.~~

> **바로잡음(2026-09-04):** ECMAScript 명세와 TC39는 ASI의 동작을 정의하지만 특정 코드 스타일을 권장하지 않는다. ESLint의 세미콜론 규칙도 설정 가능한 스타일 규칙이다. 프로젝트의 포매터·린터 규칙을 일관되게 따르고 ASI가 위험한 줄 앞에는 방어 세미콜론을 둔다.
...
> **바로잡음(2026-09-05):** 모든 문의 완료 값이 `undefined`인 것은 아니다. 예를 들어 `eval('if (true) { 1 + 2; }')`는 `3`이다. `if` 문 자체를 대입문의 오른쪽에 쓸 수는 없지만, `eval`은 실행한 코드의 완료 값을 반환할 수 있다. 이 예제는 의미 설명용이며 외부 입력을 `eval`로 실행하면 안 된다.

  ![completionvalue 1](<./modern js deep dive/5completionvalue_1.png>)

- 크롬 개발자 도구에서 표현식인 문을 실행하면 언제나 평가된 값을 반환한다.

  ![completionvalue 2](<./modern js deep dive/5completionvalue_2.png>)
