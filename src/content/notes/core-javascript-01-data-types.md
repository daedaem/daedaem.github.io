...
- **변수 영역 데이터 값에 바로 값을 대입하지 않는 이유**
  ...
  - ~~500개 변수에 숫자 5를 할당하면 숫자형 8바이트가 각각 필요해 4,000바이트이고, 5를 한 번만 저장한 뒤 2바이트 주소를 넣으면 1,008바이트가 필요하다.~~

> **바로잡음(2026-09-04):** 이 계산은 자바스크립트 명세가 보장하는 메모리 구조가 아니다. 엔진은 tagged value, 포인터 압축, 상수 재사용 등 서로 다른 표현을 쓸 수 있고 주소 크기도 2바이트로 고정되지 않는다. 아래 주소 그림은 값과 참조의 차이를 설명하는 개념 모델로만 본다.
- 데이터 할당 과정 예시
...
> **이미지 안내(2026-09-04):** 외부 블로그 이미지 임베드는 만료 가능성이 있어 제거했다. 위 코드처럼 `obj1`이 같은 객체를 가리킨 채 내부 프로퍼티만 바뀌는 상황을 나타낸 그림이었다.
...
> **이미지 안내(2026-09-04):** 외부 블로그 이미지 임베드는 제거했다. `obj1`과 `obj2`가 같은 객체를 참조하므로 `obj2.c = 20` 뒤에는 둘 다 `{ c: 20, d: "bbb" }`를 가리킨다.
...
> **바로잡음(2026-09-04):** 위 `obj1` 주석은 틀렸다. `obj2 = obj1` 뒤 두 변수는 같은 객체를 가리키므로 `obj2.c = 20`을 실행하면 `obj1.c`도 20이다.

> **이미지 안내(2026-09-04):** 외부 블로그 이미지 임베드는 제거했다. 위 코드는 객체 자체를 교체한 예제가 아니라, 공유 객체의 프로퍼티를 바꾼 예제다.
...
> **보충(2026-08-30):** 위 재귀 함수는 배열도 `{}`로 복사해서 배열이 아니게 되고, Date·Map·Set도 다루지 못한다. 지금은 브라우저와 Node.js에 내장된 **`structuredClone(target)`** 이 이런 값들을 처리하므로 먼저 검토한다. 함수와 DOM 노드 등 복제할 수 없는 값도 있어 모든 객체를 처리하는 것은 아니다.
...
- **`typeof null`이 `"object"`로 나오는 것은 초기 설계에서 남은 동작**
- ~~따라서 비교 시에 동등 연산자(`==`) 말고 일치 연산자(`===`)를 쓰자.~~

> **바로잡음(2026-09-04):** `typeof null` 문제와 `==`/`===` 선택은 직접 연결되지 않는다. null만 확인하려면 `value === null`, null과 undefined를 함께 확인하려는 의도를 명확히 했을 때는 `value == null`도 사용할 수 있다.
