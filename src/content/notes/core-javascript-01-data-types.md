...
> **보충(2026-08-30):** 이후 **BigInt**(ES2020)가 추가되어 기본형은 일곱 가지다.
- 값이 담긴 주소값 복제
- 불변성
...
- 1Byte = 8bit, 표현할 수 있는 값은 $2^8$ = 256가지
...
- 컴퓨터는 메모리에서 비어있는 공간 하나(주소 1003)를 정하고, 공간이름(식별자) a라고 지정
  ...
  ![데이터타입1 1](./coreJavaScript/데이터타입1-1.png)
...
- **변수 영역 데이터 값에 바로 값을 대입하지 않는 이유**
  - 변수에서 확보된 공간을 새로 할당된 데이터 크기에 맞게 늘리는 작업 시,
    특히, 중간에 있는 데이터를 늘려야 하는 상황에서 해당 공간보다 뒤에 저장된 데이터를 뒤로 옮기고, 이동시킨 주소를 각 식별자에 다시 연결해야 하는 작업으로 인해 컴퓨터 연산량 많아짐
...
- 데이터 할당 과정 예시
  ...
  - 저장한 문자열 주소 (@5004)를 @1003 공간에 대입
    ...
    ![데이터타입1 2](./coreJavaScript/데이터타입1-2.png)
...
- 변수와 상수 구분하는 성질은 **변경 가능성(변수 영역 메모리)**
  - **변경 가능성 -** 한 번 데이터 할당이 이루어진 변수 공간에 다른 데이터 재할당 가능 여부
...
- **얕은 복사**
  ...
  - but, 중첩 객체를 처리하지 못함(객체 안에 객체 존재하는 객체)
...
> **보충(2026-08-30):** 위 재귀 함수는 배열도 `{}`로 복사해서 배열이 아니게 되고, Date·Map·Set도 다루지 못한다. 지금은 브라우저와 Node.js에 내장된 **`structuredClone(target)`** 이 이런 경우를 모두 처리하므로 그것을 먼저 쓴다. 함수는 여전히 복사되지 않는다.
...
- 따라서, 비교 시에 동등 연산자(==) 말고 일치 연산자(===)를 쓰자
