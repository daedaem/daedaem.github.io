...
legacyPath: "/230303_코어자바스크립트 ch 7. 클래스/"
---
...
- ~~자바스크립트는 프로토타입 기반 언어라서 클래스 및 상속 개념이 없다.~~
- 프로토타입 체인을 기반으로 상속이 동작한다
- ES6부터는 그 위에 `class` 문법이 도입됐다

> **바로잡음(2026-09-04):** 자바스크립트의 `class`는 프로토타입 기반 동작을 감싼 표준 문법이고, `extends`로 상속을 표현한다. 다른 언어의 클래스 모델과 내부 구조가 같지 않다는 뜻이지 클래스와 상속 개념이 없다는 뜻은 아니다.
