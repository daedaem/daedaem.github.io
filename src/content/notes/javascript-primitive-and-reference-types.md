...
legacyPath: "/230106_js-참조형 & 원시형 데이터타입/"
---
...
> **보충(2026-08-30):** 스프레드로 만든 복사는 **얕은 복사**다. 한 단계 아래의 프로퍼티만 새로 담고, 그 값이 객체나 배열이면 여전히 같은 것을 가리킨다. `person.address.city`처럼 중첩된 값을 바꾸면 복사본에도 반영된다. 지원되는 값 전체를 깊게 복사하려면 `structuredClone(person)`을 쓸 수 있다. 함수와 DOM 노드처럼 복제할 수 없는 값도 있으므로 모든 객체에 통하는 방법은 아니다.
