...
legacyPath: "/230403_TypeScript - 3.2.Interfaces/"
---

# Interface

- 인터페이스는 JavaScript와 같은 동적 타입 언어 환경에서는 다뤄지지 않지만 정적 타입 언어인 TypeScript에서는 타입 검사가 요구되므로 인터페이스를 지원한다.
- **객체의 구체적 값이 아니라 구조를 정의하는 데 사용,**
- 인터페이스는 interface 키워드를 사용해 다음과 같이 정의하여 사용할 수 있음.
...
- 인터페이스 내에 구현이나 값이 아니라 구조나 클래스가 가져야 할 기능을 입력해야 함.
...
- 인터페이스로 하는 일은 대부분 `type` 별칭으로도 할 수 있다. 클래스가 `implements` 하는 것도 객체 타입 별칭이면 가능하다.

> **보충(2026-08-30):** 실제 차이는 둘이다. 인터페이스는 같은 이름으로 여러 번 선언하면 합쳐지고(declaration merging), `extends`로 확장한다. 타입 별칭은 합쳐지지 않고 `&`로 결합하며, 유니온·튜플·원시 타입에도 이름을 붙일 수 있다. 객체 구조에는 인터페이스, 그 외에는 타입 별칭을 쓰는 것이 관례다.
...
- ~~인터페이스 내의 속성에 readonly를 붙여 객체 내 속성이 한 번만 설정되고 읽기 전용으로 설정하여 객체 초기화되면 변경할 수 없도록 할 수 있다.~~

> **바로잡음(2026-09-05):** `readonly`는 해당 타입을 통한 재할당을 컴파일 시점에 막는다. 실행 중 객체를 동결하지 않으며, 속성이 가리키는 객체의 내부까지 불변으로 만들지도 않는다.
...
- 클래스와 달리 **인터페이스는 여러 인터페이스로부터 상속 가능하다.**
- 어떤 객체, 클래스는 Named를 구현하고 다른 객체, 클래스는 Greetable을 구현하여 greet과 name 모두를 입력하도록 할 수 있다. 이러한 경우에 인터페이스를 나누고 확장하는 것이 유용하다.
...
- 인터페이스는 함수의 구조를 정의하는 데에도 사용할 수 있다.
- 아래와 같이 이름 없는 익명 함수로 인터페이스를 함수 타입으로 사용할 수도 있다.
...
- 자바스크립트로 컴파일되고 나면 인터페이스에 관련된 코드는 찾아볼 수 없다.
- 개발 및 컴파일 도중에만 사용할 수 있는 타입스크립트 전용 기능
- 명확하게 구조화된 코드를 작성하는 데 도움이 되는 순수 개발 기능이다.

## Q&A

**Q. 클래스와 인터페이스의 차이점은?**

A. 인터페이스는 인스턴스화할 수 없고 컴파일되지 않는 반면, 클래스는 인스턴스화할 수 있으며 컴파일된다.
