...
React가 이 이름 규칙으로 "이 함수 안에서는 훅을 호출해도 된다"고 판단한다. ESLint 훅 규칙도 여기에 기댄다.
...
두 컴포넌트는 각각 **자기만의 `counter` 상태**를 갖는다. 훅이 호출될 때마다 새 상태가 만들어진다. 공유하고 싶으면 Context나 Redux를 써야 한다. 커스텀 훅은 **상태 공유가 아니라 로직 공유** 도구다.
...
4. 훅이 반환하는 함수는 `useCallback`으로 감싼다
