...
애플리케이션에서 `""`와 `null`을 구분해 다뤄도 일반적인 `VARCHAR2` 컬럼에 저장하면 둘이 합쳐진다. "빈 값으로 명시적으로 지웠다"와 "값이 없다"를 구분해야 한다면 문자열 컬럼 하나에만 의존하지 말고 별도 상태 컬럼이나 변경 명령으로 의미를 표현해야 한다.
...
업무상 두 값이 같은 의미라면 **값이 들어오는 경계에서 한쪽으로 정규화**할 수 있다. 둘이 다른 명령이나 상태를 뜻한다면 정규화하면 안 된다.
