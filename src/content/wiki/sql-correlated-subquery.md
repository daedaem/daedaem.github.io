...
`NOT EXISTS`가 빠른 이유는 **존재 여부만 확인하면 되기 때문**이다. 조건에 맞는 행을 하나 찾는 순간 그 행에 대한 판단은 끝난다. 반면 `NOT IN`은 비교할 값의 목록을 통째로 만들어야 한다.
...
- 상관 서브쿼리는 메인 쿼리의 행마다 반복 실행된다. 그런데 존재 여부만 확인하면 되므로 **조기 종료가 가능해 오히려 빠른 경우가 많다.**
