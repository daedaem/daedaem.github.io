...
memory: ''
runtime: ''
problemUrl: 'https://school.programmers.co.kr/learn/courses/30/lessons/72410'
langs: ['python']
---
```python
def solution(new_id):
...
> **바로잡음(2026-09-06):** 허용되지 않는 문자를 제거한 결과가 비었을 때 `result[0]`에서 `IndexError`가 났다. 빈 문자열에서도 안전한 `result[:1]`로 바꿔 아래의 빈 값 처리까지 진행하게 했다. 입력 `"!"`의 결과는 `"aaa"`다.
