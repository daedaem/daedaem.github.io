...
export const PROJECTS: Project[] = [
  {
    name: 'GymSpot',
    tagline: '동시 예약 경합을 어디서 막을지 정하는 중 — 아직 설계 단계다',
    period: '2026.08 – 설계 중',
    status: '설계 단계 — 요구사항과 ERD를 정리하고 프로젝트 골격까지. 예약 도메인 구현은 아직 시작하지 않았다',
    what: '생활체육센터의 대관 공간을 시간 단위로 조회·예약·관리하는 웹 애플리케이션을 만들려고 한다. 작업 중인 저장소에 있는 것은 요구사항·ERD·설계 메모와 Spring Boot·React 골격뿐이다. 코드를 쓰기 전에 "같은 시간에 두 명이 동시에 누르면 어떻게 되는가"를 먼저 정하고 있고, 아래는 그 검토 내용이다.',
    points: [
      {
        title: '검토 중 — 겹침을 검사하는 대신 겹칠 수 없는 구조로 두는 방향',
        body: '예약을 시작·종료 시각 한 쌍으로 저장하면 겹침 판정을 코드로 해야 하고, 두 요청이 동시에 검사를 통과하는 창이 생긴다. 예약을 30분 슬롯 행으로 펼쳐 저장하고 `UNIQUE(공간, 슬롯시각)` 제약을 거는 쪽을 우선 검토하고 있다. 이렇게 두면 겹침은 판정할 대상이 아니라 DB가 거부하는 일이 된다. 아직 구현해 검증한 것은 아니다.',
      },
      {
        title: '검토 중 — 애플리케이션 검사는 남기되 역할을 바꾸는 것',
        body: '겹침 검사를 없앨 생각은 없다. 다만 정합성을 지키는 장치가 아니라 사용자에게 미리 알려 주는 UX 장치로 성격을 옮기려고 한다. 최종 판단은 DB 제약이 하고, 검사는 사용자가 실패를 늦게 아는 것을 줄여 주는 역할이다.',
      },
      {
        title: '지금 상태와 다음',
        body: '요구사항·ERD·설계 원칙 문서와 백엔드·프론트엔드 골격까지 만들어 둔 상태다. 예약 도메인, 화면, 관리자 기능은 시작 전이다. 다음은 스키마를 확정하고 경합 상황을 실제로 만들어 제약이 의도대로 거부하는지 확인하는 테스트를 쓰는 것이다. 진행되는 대로 여기에 다시 적는다.',
      },
    ],
    stack: [
      'Java 21',
