...
export const PROJECTS: Project[] = [
  {
    name: 'GymSpot',
    tagline: '동시 예약 경합을 코드 검사가 아니라 DB 제약으로 막는 설계',
    period: '2026.08 – 설계 중',
    status:
      '설계 단계 — 요구사항과 ERD를 정리하고 프로젝트 골격까지. 예약 도메인 구현은 아직 시작하지 않았다',
    what: '생활체육센터의 대관 공간을 시간 단위로 조회·예약·관리하는 웹 애플리케이션이다. 코드를 쓰기 전에 "같은 시간에 두 명이 동시에 누르면 어떻게 되는가"를 먼저 정하고 있고, 아래는 그 검토 내용이다.',
    points: [
      {
        title: '검토 중 — 겹침을 검사하는 대신 겹칠 수 없는 구조로 두는 방향',
        body: '예약을 시작·종료 시각 한 쌍으로 저장하면 겹침 판정을 코드로 해야 하고, 두 요청이 동시에 검사를 통과하는 창이 생긴다. 예약을 30분 슬롯 행으로 펼쳐 저장하고 `UNIQUE(공간, 슬롯시각)` 제약을 거는 쪽을 우선 검토하고 있다. 이렇게 두면 겹침은 판정할 대상이 아니라 DB가 거부하는 일이 된다.',
      },
      {
