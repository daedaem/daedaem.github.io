export const SITE = {
  ...
  /** 링크 공유 이미지 제목. 직무는 카드 kicker(SITE.role)가 맡는다. 화면에서는 쓰지 않는다. */
  identityTitle: '조해성의 기술 블로그',
  /** 홈 H1(첫 화면 머리글)이자 링크 공유 이미지 부제. */
  motto: '증상이 아니라 원인을 고칩니다',
  /** 홈 H1에서 강조색으로 칠하는 낱말. motto 안에 있어야 한다(없으면 강조 없이 그린다). */
  mottoAccent: '원인',
  /** 직무. 화면(홈·소개·글 메타·글쓴이 카드)과 JSON-LD가 같은 값을 쓴다. */
  role: '백엔드 개발자',
  /** 소개 '환경' 값이자 홈 첫 화면의 주력 기술 한 줄. 문장을 바꾸면 두 화면이 함께 바뀐다. */
  environment: 'Java·Spring·Oracle 중심, C#/.NET 병행',
  /** 홈 첫 화면에서 motto 아래에 붙는 설명. 무슨 문제를 어떻게 다루고 무엇을 남기는지. */
  intro:
    '느린 조회와 어긋난 데이터처럼 운영 중 만난 문제를 화면에서 DB까지 로그와 데이터 흐름을 따라 추적합니다. 무엇을 확인했고 왜 그 방법을 골랐는지 기록합니다.',
  description:
    '레거시 시스템을 개발·운영하는 백엔드 개발자 조해성의 기록. 데이터 정합성, 조회 성능, 시스템 연동을 개선하며 내린 판단과 결과를 남깁니다.',
  author: '조해성',
